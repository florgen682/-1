...
victory_screen = Entity(model='quad', color=color.rgba(0, 0, 0, 216), scale=(2, 2), parent=camera.ui, enabled=False,
                        z=-1)
victory_text = Text(text='МИССИЯ ВЫПОЛНЕНА!', origin=(0, 0), scale=2, color=color.gold, parent=victory_screen, y=0.1)
...
def update():
    global current_cargo, current_speed_level
    if victory_screen.enabled: return
    ...
    if held_keys['w']:
        hit_info = raycast(truck.world_position + Vec3(0, 0.3, 0), truck.forward, distance=2.7,
                           ignore=(truck, *back_wheels, *front_wheels))
        if hit_info.hit:
...
def input(key):
    global current_cargo, current_speed_level, score
    if victory_screen.enabled: return
    ...
    if key == 'space':
        if not current_cargo:
            ...
            if dist_to_zone < 6.5:
                ...
                floor_hit = raycast(current_cargo.world_position, Vec3(0, -1, 0), distance=30,
                                    ignore=(current_cargo, truck))
                if floor_hit.hit:
                    current_cargo.y = floor_hit.world_point.y + 1.5
                else:
                    current_cargo.y = 1.5
                current_cargo.collider = 'box'
                current_cargo = None
